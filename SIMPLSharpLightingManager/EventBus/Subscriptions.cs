namespace Easy.MessageHub
{
    ...
    internal static class Subscriptions
    {
        ...
        internal static Guid Register<T>(TimeSpan throttleBy, Action<T> action)
        {
            ...
            var subscription = new Subscription(type, key, throttleBy, action, null);
        ...
        internal static Guid Register<T>(TimeSpan throttleBy, Action<T> action, Predicate<T> filter)
        {
            var type = typeof(T);
            var key = Guid.NewGuid();
            var subscription = new Subscription(type, key, throttleBy, action, filter);
